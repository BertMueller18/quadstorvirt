...
int dump_disk(FILE *fp, struct physdisk *disk, uint32_t bid)
{
	...
	fprintf (fp, "dedupe_blocks: %"PRIu64"\n", disk->dedupe_blocks);
	fprintf (fp, "total_blocks: %"PRIu64"\n", disk->total_blocks);
	fprintf (fp, "uncompressed_size: %"PRIu64"\n", disk->uncompressed_size);
	fprintf (fp, "compressed_size: %"PRIu64"\n", disk->compressed_size);
	fprintf (fp, "compression_hits: %"PRIu64"\n", disk->compression_hits);
	fprintf (fp, "raiddisk: %d\n", disk->raiddisk);
	fprintf (fp, "initialized: %d\n", disk->initialized);
	fprintf (fp, "ddmaster: %d\n", disk->ddmaster);
	fprintf (fp, "log_disk: %d\n", disk->log_disk);
	fprintf (fp, "ha_disk: %d\n", disk->ha_disk);
	fprintf (fp, "unmap: %d\n", disk->unmap);
	fprintf (fp, "write_cache: %d\n", disk->write_cache);
	fprintf (fp, "enable_comp: %d\n", disk->enable_comp);
...
tl_common_scan_physdisk(void)
{
	...
	while (fgets(buf, sizeof(buf), fp) != NULL)
	{
		...
		if (retval == 0 && is_quadstor_vdisk(vendor, serialnumber))
			continue;
		if (retval != 0)
			serial_len = 0;
