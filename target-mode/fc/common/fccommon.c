...
struct device_info *device_list[TL_MAX_DEVICES];
...
static int fcbridge_new_device_cb(struct tdisk *device);
static int fcbridge_remove_device_cb(struct tdisk *device, int tid, void *hpriv);
static void fcbridge_disable_device_cb(struct tdisk *device, int tid, void *hpriv);
...
fcbridge_locate_dinfo(int bus)
{
	...
	if (unlikely(bus >= TL_MAX_DEVICES))
		return NULL;
...
static void 
fcbridge_disable_device_cb(struct tdisk *device, int tid, void *hpriv)
{
...
static int
fcbridge_remove_device_cb(struct tdisk *device, int tid, void *hpriv)
{
	struct device_info *dinfo;
	uint32_t base_id;
...
static int
fcbridge_new_device_cb(struct tdisk *device)
{
...
fcbridge_route_cmd_post(struct qsio_scsiio *ctio)
{
	...
	case COMPARE_AND_WRITE:
		...
		num_blocks = 0;
		retval = (*icbs.ctio_write_length)(ctio, device, &num_blocks, &dxfer_len);
		...
		retval = (*icbs.device_allocate_buffers)(ctio, num_blocks, Q_NOWAIT);
		if (unlikely(retval != 0)) {
			ctio_construct_sense(ctio, SSD_CURRENT_ERROR, SSD_KEY_ABORTED_COMMAND, 0, COMMAND_PHASE_ERROR_ASC, COMMAND_PHASE_ERROR_ASCQ);
			(*icbs.device_send_ccb)(ctio);
			return;
...
fcbridge_cmd_report_luns(struct fcbridge *fcbridge, struct qsio_scsiio *ctio)
{
	...
	length = min_t(uint32_t, (((TL_MAX_DEVICES + 1) * 8) + 16), allocation_length);
...
linker_detach(linker_file_t lf, void *arg)
{
	...
	if (strcmp(lf->filename, "coredev.ko"))
		return 0;
