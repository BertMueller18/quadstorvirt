...
ddtable_ddlookup_list_last(struct ddtable *ddtable, struct ddlookup_list *ddlookup_list, int *error)
{
	...
	while (ddlookup) {
		if (ddlookup_check_read_io(ddtable, ddlookup, 0))
			DD_INC(async_load, 1);
...
ddtable_load_peers(struct ddtable *ddtable)
{
	...
	while (!kernel_thread_check(&ddtable->flags, DDTABLE_LOAD_EXIT)) {
		...
		for (i = 0; i < ddtable->max_roots && !kernel_thread_check(&ddtable->flags, DDTABLE_LOAD_EXIT); i++) {
			...
			if (ddlookup_check_read_io(ddtable, next, 0)) {
				DD_INC(async_load, 1);
			}
...
int
ddtable_load_status(struct ddtable *ddtable)
{
	return (atomic_test_bit(DDTABLE_LOAD_DONE, &ddtable->flags));
}
...
ddtable_ddlookup_get_next(struct ddtable *ddtable, struct ddlookup_list *ddlookup_list, struct ddtable_ddlookup_node *child, uint64_t next_block, int *error)
{
	...
	if (ddlookup_check_read_io(ddtable, next, 1))
		DD_INC(hash_load, 1);
	return next;
}
...
static int
ddlookup_check_read_io(struct ddtable *ddtable, struct ddtable_ddlookup_node *ddlookup, int start_io)
{
	struct tpriv priv = { 0 };
	int retval;
...
ddtable_ddlookup_find_node(struct ddtable *ddtable, uint64_t node_block)
{
	...
	node_unlock(node);
	if (ddlookup_check_read_io(ddtable, ddlookup, 1))
		DD_INC(find_load, 1);
