...
static void
registrant_unit_attention(struct tdisk *tdisk, struct registration *registration, uint8_t asc, uint8_t ascq)
{
...
static void
registrants_unit_attention(struct tdisk *tdisk, uint8_t asc, uint8_t ascq, uint64_t i_prt[], uint64_t t_prt[], uint8_t init_int, int skip)
{
...
static void
persistent_reservation_remove_registration(struct tdisk *tdisk, struct registration *registration)
{
	struct reservation *reservation = &tdisk->reservation;
...
int
persistent_reservation_handle_register(struct tdisk *tdisk, struct qsio_scsiio *ctio)
{
	struct reservation *reservation = &tdisk->reservation;
	...
	SLIST_FOREACH(tmp, &reservation->registration_list, r_list) {
		...
		retval = tdisk_mirror_cmd_generic_nocheck(tdisk, ctio, 1);
...
int
persistent_reservation_handle_register_and_ignore(struct tdisk *tdisk, struct qsio_scsiio *ctio)
{
	struct reservation *reservation = &tdisk->reservation;
	struct registration *tmp, *registration, *prev = NULL;
	struct reservation_parameter *param;
...
int
persistent_reservation_handle_reserve(struct tdisk *tdisk, struct qsio_scsiio *ctio)
{
	struct reservation *reservation = &tdisk->reservation;
...
int
persistent_reservation_handle_release(struct tdisk *tdisk, struct qsio_scsiio *ctio)
{
	struct reservation *reservation = &tdisk->reservation;
	uint8_t *cdb = ctio->cdb;
	struct reservation_parameter *param;
...
int
persistent_reservation_handle_clear(struct tdisk *tdisk, struct qsio_scsiio *ctio)
{
	struct reservation *reservation = &tdisk->reservation;
	struct reservation_parameter *param;
	uint64_t key;
...
static void
persistent_reservation_abort_tasks(struct tdisk *tdisk, uint64_t i_prt[], uint64_t t_prt[], uint16_t r_prt, int init_int)
{
	struct initiator_state *istate;
	...
	gdevq_abort_tasks_other_initiators(i_prt, t_prt, init_int);
...
void
reservation_spec_fill(struct reservation *reservation, struct reservation_spec *reservation_spec)
{
	reservation_spec->generation = reservation->generation;
	reservation_spec->is_reserved = reservation->is_reserved;
	reservation_spec->type = reservation->type;
	reservation_spec->persistent_type = reservation->persistent_type;
	reservation_spec->init_int = reservation->init_int;
	port_fill(reservation_spec->i_prt, reservation->i_prt);
	port_fill(reservation_spec->t_prt, reservation->t_prt);
	reservation_spec->r_prt = reservation->r_prt;
	reservation_spec->persistent_key = reservation->persistent_key;

}

void
reservation_spec_copy(struct reservation *reservation, struct reservation_spec *reservation_spec)
{
	reservation->generation = reservation_spec->generation;
	reservation->is_reserved = reservation_spec->is_reserved;
	reservation->type = reservation_spec->type;
	reservation->persistent_type = reservation_spec->persistent_type;
	reservation->init_int = reservation_spec->init_int;
	port_fill(reservation->i_prt, reservation_spec->i_prt);
	port_fill(reservation->t_prt, reservation_spec->t_prt);
	reservation->r_prt = reservation_spec->r_prt;
	reservation->persistent_key = reservation_spec->persistent_key;
}

void
registration_spec_add(struct tdisk *tdisk, struct reservation *reservation, struct registration_spec *registration_spec)
{
	struct registration *registration;

	registration = zalloc(sizeof(struct registration), M_RESERVATION, Q_WAITOK);
	registration->key = registration_spec->key;
	port_fill(registration->i_prt, registration_spec->i_prt);
	port_fill(registration->t_prt, registration_spec->t_prt);
	registration->r_prt = registration_spec->r_prt;
	registration->init_int = registration_spec->init_int;
	tdisk_reservation_lock(tdisk);
	SLIST_INSERT_HEAD(&reservation->registration_list, registration, r_list);
	reservation_spec_copy(reservation, &registration_spec->reservation_spec);
	tdisk_reservation_unlock(tdisk);
}

void
registration_spec_clear(struct tdisk *tdisk, struct reservation_spec *reservation_spec)
{
	struct reservation *reservation = &tdisk->sync_reservation;
...
void
registration_spec_modify(struct tdisk *tdisk, struct registration_spec *registration_spec)
{
	struct registration *tmp;
	struct reservation *reservation = &tdisk->sync_reservation;
	...
	tdisk_reservation_lock(tdisk);
	SLIST_FOREACH(tmp, &reservation->registration_list, r_list) {
		if (!iid_equal(tmp->i_prt, tmp->t_prt, tmp->init_int, registration_spec->i_prt, registration_spec->t_prt, registration_spec->init_int))
			continue;

		tmp->key = registration_spec->key;
		break;
	}
	reservation_spec_copy(reservation, &registration_spec->reservation_spec);
	tdisk_reservation_unlock(tdisk);
...
void
registration_spec_remove(struct tdisk *tdisk, struct reservation *reservation, struct registration_spec *registration_spec)
{
	struct registration *tmp, *prev = NULL;
