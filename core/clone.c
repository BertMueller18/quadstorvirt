...
clone_amap_table_get(struct tdisk *dest_tdisk, struct amap_table_group *dest_group, uint32_t group_id, uint32_t group_offset, uint32_t atable_id, struct write_list *wlist)
{
	...
	if (!block && !dest_amap_table) {
		wlist_lock(wlist);
		retval = amap_table_init(dest_tdisk, dest_group, atable_id, &wlist->meta_index_info_list, NULL);
		wlist_unlock(wlist);
...
tdisk_needs_write_bmap(struct tdisk *tdisk)
{
	...
	if (!tdisk_mirroring_disabled(tdisk) && !atomic_test_bit(MIRROR_FLAGS_NEED_RESYNC, &mirror_state->mirror_flags))
		return 0;
...
static void
amap_table_write_bmap_check(struct tdisk *tdisk, struct amap_table *amap_table)
{
...
static void
amap_write_bmap_check(struct tdisk *tdisk, struct amap *amap)
{
...
__amap_mirror_check(struct tdisk *src_tdisk, struct amap *src_amap, int isnew)
{
	...
	atomic_inc(&src_tdisk->clone_busy);
	tdisk_bmap_unlock(src_tdisk);
	...
	atomic_dec(&src_tdisk->clone_busy);
	return;
}
...
__amap_clone_check(struct tdisk *dest_tdisk, struct amap *src_amap, int isnew)
{
	...
	atomic_inc(&dest_tdisk->clone_busy);
	tdisk_bmap_unlock(dest_tdisk);
	...
	if (unlikely(!dest_amap_table)) {
		...
		vm_pg_free(metadata);
		atomic_dec(&dest_tdisk->clone_busy);
		return;
	}
	...
	dest_amap = amap_new(dest_amap_table, src_amap->amap_id, src_amap->amap_idx, &dest_tdisk->clone_wlist->meta_index_info_list, NULL, &error);
...
{
	amap_table_write_bmap_check(src_tdisk, src_amap_table);
...
{
	amap_write_bmap_check(src_tdisk, src_amap);
	if (!tdisk_in_cloning(src_tdisk) && !tdisk_in_mirroring(src_tdisk))
		return;
...
clone_list_wait(struct clone_info *clone_info, struct tdisk *tdisk)
{
	...
	while (atomic_read(&tdisk->clone_busy))
		pause("psg", 100);
...
amap_table_clone(struct clone_info *clone_info, struct tdisk *dest_tdisk, struct amap_table *dest_amap_table, struct amap_table *src_amap_table, struct write_list *wlist, uint32_t group_id)
{
	...
	for (i = 0; i < todo; i++, amap_id++) {
		...
		dest_amap = amap_new(dest_amap_table, amap_id, i, &wlist->meta_index_info_list, NULL, &error);
		if (unlikely(!dest_amap)) {
			tdisk_set_clone_error(dest_tdisk);
			amap_table_unlock(dest_amap_table);
			atomic_clear_bit_short(AMAP_META_DATA_BUSY, &src_amap->flags);
