...
enum {
	...
	NODE_STATUS_DO_LOCAL_READ	= 0xED,
};
...
int clone_list_wait(struct clone_info *clone_info, struct tdisk *tdisk);
...
int node_send_write_io(struct clone_data *clone_data, struct qsio_scsiio *ctio, struct node_comm *comm, struct node_sock *sock, struct node_msg *msg, struct pgdata **pglist, int pglist_cnt, int timeout, int async);
...
int node_usr_send_job_completed(struct usr_job *job_info, int status);
