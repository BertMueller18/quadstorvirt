...
#endif
		if (sgentry->offset || ((sgentry->length & LBA_MASK) && ((j + 1) != scsi_sg_count(SCpnt))))
			return 0;

		if (sgentry->length > LBA_SIZE) {
			pagestruct_t *start, *end, *next;

			start = sg_page(sgentry);
			end = virt_to_page(page_address(start) + (LBA_SIZE - 1));
			next = virt_to_page(page_address(start) + LBA_SIZE);
...
#endif
		pgtmp = pglist[i];
		page = sg_page(sgentry) + sg_idx;
		pgtmp->page = page;
		i++;
		if (i == ctio->pglist_cnt)
			break;
		sg_idx++;
		sgoffset += LBA_SIZE;
...
static struct qs_kern_cbs kcbs = {
	...
	.ctio_bio_aligned	= ctio_bio_aligned,
	.ctio_map_bio		= ctio_map_bio,
	.copy_in_request_buffer = copy_in_request_buffer,
	.copy_in_request_buffer2 = copy_in_request_buffer2,
...
#endif
{
	...
	case TLTARGIOCNODESTATUS:
		node_config = malloc(sizeof(*node_config), M_QUADSTOR, M_NOWAIT);
		if (!node_config) {
			retval = -ENOMEM;
			break;
		}
	...
	case TLTARGIOCSETMIRRORROLE:
		tdisk_info = malloc(sizeof(struct tdisk_info), M_QUADSTOR, M_NOWAIT);
		if (!tdisk_info) {
			retval = -ENOMEM;
			break;
		}
