...
struct qs_interface_cbs {
	...
	int (*new_device) (struct tdisk *);
	int (*update_device) (struct tdisk *, int tidk, void *hpriv);
	int (*remove_device) (struct tdisk *, int tid, void *hpriv);
	void (*disable_device) (struct tdisk *, int tid, void *hpriv);
	...
	int (*ctio_write_length) (struct qsio_scsiio *, struct tdisk *, uint32_t *, int *);
	void (*pgdata_free_page) (struct pgdata *);

	void (*write_lun)(uint32_t, uint8_t *);
	uint32_t (*bus_from_lun)(uint16_t ilun);
	struct tdisk* (*get_device)(uint32_t);
	uint32_t (*device_tid) (struct tdisk *);
	uint64_t (*device_end_lba) (struct tdisk *);
	uint32_t (*device_lba_shift) (struct tdisk *);
	char* (*device_name)(struct tdisk *);
	void (*device_set_vhba_id)(struct tdisk *, int, int);
	void (*device_set_hpriv)(struct tdisk *, void *);
	void (*device_send_ccb) (struct qsio_scsiio *);
	void (*device_send_notify) (struct qsio_immed_notify *);
	int (*device_istate_queue_ctio) (struct tdisk *, struct qsio_scsiio *);
	int (*device_istate_abort_task) (struct tdisk *, uint64_t[], uint64_t[], int, uint32_t);
	void (*device_istate_abort_task_set) (struct tdisk *, uint64_t[], uint64_t[], int);
	int (*device_queue_ctio) (struct tdisk *, struct qsio_scsiio *);
	...
	int (*device_check_cmd) (uint8_t);
	int (*device_allocate_buffers) (struct qsio_scsiio *, uint32_t, allocflags_t);
	int (*device_allocate_cmd_buffers) (struct qsio_scsiio *, allocflags_t);
	void (*device_target_reset) (struct tdisk *vdevice, uint64_t i_prt[], uint64_t t_prt[], uint8_t init_int);
	void (*device_free_initiator) (uint64_t[], uint64_t[], int, struct tdisk *);
...
struct qs_kern_cbs {
	...
	wait_compl_t* (*wait_compl_alloc) (const char *name);
	void (*wait_compl_free) (wait_compl_t *comp);
	void (*init_wait_compl) (wait_compl_t *comp);
	void (*set_wait_compl) (wait_compl_t *comp);
	void (*wait_complete) (wait_compl_t *comp);
	void (*wait_complete_all) (wait_compl_t *comp);
	void (*wait_for_done) (wait_compl_t *comp);
	long (*wait_for_done_timeout) (wait_compl_t *comp, long timo);
	wait_chan_t* (*wait_chan_alloc) (const char *name);
	void (*wait_chan_free) (wait_chan_t *chan);
	void (*chan_lock) (wait_chan_t *chan);
	void (*chan_unlock) (wait_chan_t *chan);
	void (*chan_lock_intr) (wait_chan_t *chan, void *);
	void (*chan_unlock_intr) (wait_chan_t *chan, void *);
	void (*chan_wakeup_one) (wait_chan_t *chan);
	void (*chan_wakeup_one_unlocked) (wait_chan_t *chan);
	void (*chan_wakeup_one_nointr) (wait_chan_t *chan);
	void (*chan_wakeup) (wait_chan_t *chan);
	void (*chan_wakeup_unlocked) (wait_chan_t *chan);
	void (*chan_wakeup_nointr) (wait_chan_t *chan);
	void (*wait_on_chan_sig) (wait_chan_t *chn, int intr);
	void (*wait_on_chan) (wait_chan_t *chn, void *, int intr);
	long (*wait_on_chan_timeout) (wait_chan_t *chn, void *, long timo);
	...
	int (*vdisk_clone)(struct clone_config *);
	int (*vdisk_clone_status)(struct clone_config *);
	int (*vdisk_clone_cancel)(struct clone_config *);
	int (*vdisk_mirror)(struct clone_config *);
	int (*vdisk_mirror_status)(struct clone_config *);
	int (*vdisk_mirror_cancel)(struct clone_config *);
	int (*vdisk_mirror_remove)(struct clone_config *);
	int (*node_config)(struct node_config *);
	...
	int (*target_delete_vdisk)(struct tdisk_info *, unsigned long);
	int (*target_delete_vdisk_post)(struct tdisk_info *, unsigned long);
...
ctio_construct_sense(struct qsio_scsiio *ctio, uint8_t error_code, uint8_t sense_key, uint32_t info, uint8_t asc, uint8_t ascq)
{
	...
	if (ctio->cdb[0])
	{
		printf("construct_sense: cmd %x sense_key %x asc %x ascq %x\n", ctio->cdb[0], sense_key, asc, ascq);
		print_cdb(ctio->cdb);
	}
#endif
